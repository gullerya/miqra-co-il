...
const
	widgetProfitsModel = DataTier.ties.create('widgetProfits', {
		onEntryEnter: event => {
			...
			widgetProfitsModel.currentHint = t.fundTotal;
			t.getRootNode().querySelector('.drawing .' + t.className).classList.add('selected');
		},
		onEntryLeave: event => {
			const t = event.target;
			widgetProfitsModel.currentHint = widgetProfitsModel.total;
...
template.innerHTML = `
	...
	<div class="column chart">
		<div class="drawing-section">
			...
			<span-currency class="hint nis" data-tie="widgetProfits:currentHint"></span-currency>
		</div>
		<div class="titles">
			<span class="title main" data-tie="l10n:widgetProfits.mainTitle"></span>
			<span class="title sub" data-tie="l10n:widgetProfits.subTitle"></span>
		</div>
	</div>

	<div class="column legend">
		<template class="magician" is="data-tier-item-template" data-tie="widgetProfits:entries">
			<div data-tie="item:type => className, item:total => fundTotal, widgetProfits:onEntryEnter => onmouseenter, widgetProfits:onEntryLeave => onmouseleave">
	...
	updateView() {
		if (widgetProfitsModel.entries && widgetProfitsModel.entries.length) {
			const
				...
				width = 10,
				entriesCnt = widgetProfitsModel.entries.length;
			let lastPos = 1;
			widgetProfitsModel.entries
				.filter(entry => entry && entry.type && entry.total)
				.forEach(entry => {
					const
						to = lastPos + Math.max(entry.total / widgetProfitsModel.total * (360 - entriesCnt) - 1, 2);
