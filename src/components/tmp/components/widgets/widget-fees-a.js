...
template.innerHTML = `
	<style>
		...
		.pension .color-hint {
			background-color: #3ea47c;
		}
		.study .color-hint {
			background-color: #5ec49c;
		}
	...
	<h5 class="main" data-tie="l10n:widgetFees.mainTitle"></h5>
	<span class="subtitle" data-tie="l10n:widgetFees.subTitle"></span>
	<span-currency class="total-sum" data-tie="widgetFees:total"></span-currency>

	<div class="details">
		<div class="chart">
			<svg class="pie"></svg>
			<svg-icon class="icon" type="fee"></svg-icon>
		</div>
...
initComponent('widget-fees', class extends ComponentBase {
	updateView() {
		if (widgetFeesModel.entries && widgetFeesModel.entries.length) {
			...
			widgetFeesModel.entries.sort((i1, i2) => i1.order < i2.order ? -1 : 1);
			widgetFeesModel.entries
				.filter(entry => entry && entry.type && entry.total)
				.forEach(entry => {
					const
						to = lastPos + Math.max(entry.total / widgetFeesModel.total * 360, 2);
					if (lastPos === 0 && to === 360) {
						lastPos = 1;
					}
			...
			this.drawArc(svg, centerX, centerX, centerX - width / 2, width, 0, 2, widgetFeesModel.entries[0].type);
		}
	}
