...
const
	widgetDepositsModel = DataTier.ties.create('widgetDeposits', {
		onEntryEnter: event => {
			...
			widgetDepositsModel.currentHint = t.fundTotal;
			t.getRootNode().querySelector('.drawing .' + t.className).classList.add('selected');
		},
		onEntryLeave: event => {
			const t = event.target;
			widgetDepositsModel.currentHint = widgetDepositsModel.total;
...
template.innerHTML = `
	...
	<div class="column chart">
		<div class="drawing-section">
			...
			<span-currency class="hint nis" data-tie="widgetDeposits:currentHint"></span-currency>
		</div>
		<div class="titles">
			<span class="title main" data-tie="l10n:widgetDeposits.mainTitle"></span>
			<span class="title sub" data-tie="l10n:widgetDeposits.subTitle"></span>
		</div>
	</div>

	<div class="column legend">
		<template class="magician" is="data-tier-item-template" data-tie="widgetDeposits:entries">
			<div data-tie="item:type => className, item:total => fundTotal, widgetDeposits:onEntryEnter => onmouseenter, widgetDeposits:onEntryLeave => onmouseleave">
	...
	updateView() {
		if (widgetDepositsModel.entries && widgetDepositsModel.entries.length) {
			const
				...
				width = 10,
				entriesCnt = widgetDepositsModel.entries.length;
			let lastPos = 1;
			widgetDepositsModel.entries
				.filter(entry => entry && entry.type && entry.total)
				.forEach(entry => {
					const
						to = lastPos + Math.max(entry.total / widgetDepositsModel.total * (360 - entriesCnt) - 1, 2);
